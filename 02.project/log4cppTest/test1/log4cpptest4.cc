...
void test0() 
{
	...
	playout3->setConversionPattern("%d %c [%p] %m%n");
	/* RollingFileAppender * prollingfileapp = */
	//auto自动推导类型
	auto prollingfileapp =
		new RollingFileAppender("rollinglog", 
				"rollingwd.log", 5000, 3);
	prollingfileapp->setLayout(playout3);
	...
	root.addAppender(prollingfileapp);
	...
	//函数名用来设置每一条日志的优先级，
	//并进行输出
	//printf("this is a error message");
